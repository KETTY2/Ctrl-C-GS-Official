...
// -------- float3 helpers --------
__device__ __forceinline__ float3 add3(float3 a, float3 b){ return make_float3(a.x+b.x, a.y+b.y, a.z+b.z); }
__device__ __forceinline__ float3 sub3(float3 a, float3 b){ return make_float3(a.x-b.x, a.y-b.y, a.z-b.z); }
__device__ __forceinline__ float3 mul3(float s, float3 v){ return make_float3(s*v.x, s*v.y, s*v.z); }
...
// quat_to_normal forward:
// n = { 2(xz + wy), 2(yz - wx), 1 - 2(x^2 + y^2) }
...
__device__ void preprocess_neighbor_effect_backward_exact_forwardmatch(
    ...
    // ---- forward replay: compute exact percent / indirect with same stopping ----
    for (int dz = -1; dz <= 1; ++dz) {
        for (int dy = -1; dy <= 1; ++dy) {
            for (int dx = -1; dx <= 1; ++dx) {
                int h = compute_hash(gx + dx, gy + dy, gz + dz, hashmap_size);

                float start = starts[h];
                float end   = ends[h];

                for (int i = (int)start; !done0 && i < (int)end; ++i) {
                    if (i == idx) continue;
                    ...
                    float t = expf(-0.1f * dist_sq);
                    percent += t * fabsf(val);

                    if (cos_area <= 0.f) {
                        float k = t * neighbor_effects[i] * 0.1f / PI_F;
                        indirect += k * val;
                    }

                    if (percent > 0.999f) done0 = true;
                }
            }
        }
    }
    ...
    // final output:
    // y = my_envlight * (1 - percent) + indirect
    // upstream g = dL/dy
    float dL_dpercent  = -g * my_envlight;
    float dL_dindirect =  g;
    float dL_denv      =  g * (1.f - percent);

    // envlight gradient
    atomicAdd(&dL_dgaussian_envlight[idx], dL_denv);
    ...
    // ---- replay again and backprop exactly through active neighbors only ----
    done0 = false;
    percent = 0.f;  // same stopping criterion reproduction
        ...
        for (int dy = -1; dy <= 1; ++dy) {
            for (int dx = -1; dx <= 1; ++dx) {
                ...
                for (int i = (int)start; !done0 && i < (int)end; ++i) {
                    ...
                    bool dir = (cos_dir <= 0.f);
                    if (!dir) continue;  // exact match: branch derivative = 0
                    ...
                    float percent_contrib = t * fabsf(val);
                    bool indirect_active = (cos_area <= 0.f);

                    // ---- local partials wrt t, val, neighbor_effect ----
                    float dL_dt =
                        dL_dpercent * fabsf(val);

                    float dL_dval =
                        dL_dpercent * t * sign_not_zero(val);

                    if (indirect_active) {
                        const float c = 0.1f / PI_F;
                        dL_dt  += dL_dindirect * neighbor_effects[i] * c * val;
                        dL_dval += dL_dindirect * t * neighbor_effects[i] * c;
                        float dL_dnei = dL_dindirect * t * c * val;
                        atomicAdd(&dL_dneighbor_effects[i], dL_dnei);
                    }
                    ...
                    if (raw2 > -1.f && raw2 < 1.f) {
                        ...
                        dL_draw2 = 0.f;  // exact forward-match subgradient
                    }
                    ...
                    // ---- t = exp(-0.1 dist_sq) ----
                    float dL_ddist_sq = dL_dt * (-0.1f * t);
	...
	float* __restrict__ dL_dneighbor_shade)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			T = T / (1.f - alpha);
			float dchannel_dcolor = alpha * T *effect;
			float dchannel_dneighbor_shade =0;
			float w = alpha * T;
			float shaded_w = w * effect;
			// Propagate gradients to per-Gaussian colors and keep
			// gradients w.r.t. alpha (blending factor for a Gaussian/pixel
			// pair).
			float dL_dalpha = 0.0f;
			float dL_dneighbor_shade_ =0.0f;
			int global_id = collected_id[j];
			for (int ch = 0; ch < C; ch++)
			{
				const float c = collected_colors[ch * BLOCK_SIZE + j];
				// Update last color (to be used in the next iteration)
				accum_rec[ch] = last_alpha *last_shade* last_color[ch] + (1.f - last_alpha) * accum_rec[ch];
				last_color[ch] = c;

				const float dL_dchannel = dL_dpixel[ch];
				dL_dalpha += (c * effect - accum_rec[ch]) * dL_dchannel;
				// Update the gradients w.r.t. color of the Gaussian. 
				// Atomic, since this pixel is just one of potentially
				// many that were affected by this Gaussian.
				dchannel_dneighbor_shade += alpha * T * c;
				dL_dneighbor_shade_ += dchannel_dneighbor_shade * dL_dchannel;
				atomicAdd(&(dL_dcolors[global_id * C + ch]), dchannel_dcolor * dL_dchannel);
			}
			atomicAdd(&(dL_dneighbor_shade[global_id]), dL_dneighbor_shade_);
			float dL_dz = 0.0f;
			float dL_dweight = 0;
#if RENDER_AXUTILITY
