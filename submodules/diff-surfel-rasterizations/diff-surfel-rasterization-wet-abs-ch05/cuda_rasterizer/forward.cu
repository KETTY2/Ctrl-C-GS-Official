...
__device__ EffectResult preprocess_neighbor_effect_kernel(
    ...
    for(int dx=-1; dx<=1; ++dx){
		...
		for(int i=start; !done0 && i<end; ++i){
			...
			if(cos_area <= 0.f){
				float k = t * neighbor_effects[i]* 0.1 / PI_F ;
				indirect += k * val;
			}
	...
	effect_accum += my_envlight*(1-percent)+indirect;
	...
	float* __restrict__ out_weight)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < CHANNELS; ch++)
				C[ch] += features[collected_id[j] * CHANNELS + ch] *shaded_w ;
			T = test_T;
	...
	if (inside)
	{
		...
		out_others[pix_id + NEIGHBOR_EFFECT * H * W] = accumulated_shade;//1일수록 밝다.PIXIL별 SHADE 
