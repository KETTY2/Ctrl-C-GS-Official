	...
	const bool debug)
{
  ...
  CHECK_INPUT(gaussian_envlight);
  CHECK_INPUT(neighbor_effects);
  CHECK_INPUT(starts);
  CHECK_INPUT(ends);
  CHECK_INPUT(densities);
  CHECK_INPUT(car_image_masks);
  CHECK_INPUT(car_gaussian_masks);
  ...
  torch::Tensor out_others = torch::full({3+3+1+1+2, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		gaussian_envlight.contiguous().data<float>() ,
		neighbor_effects.contiguous().data<float>(),//new
		starts.contiguous().data<float>(),
		ends.contiguous().data<float>(),   
		densities.contiguous().data<float>(),
		car_image_masks.contiguous().data<float>(),
		car_gaussian_masks.contiguous().data<float>() ,
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dgaussian_envlight = torch::zeros({P, 1}, means3D.options());
  torch::Tensor dL_dneighbor_effects= torch::zeros({P, 1}, means3D.options());
  torch::Tensor dL_dneighbor_shade = torch::zeros({P, 1}, means3D.options());
  ...
  {  
	  ...
	  dL_dgaussian_envlight.contiguous().data<float>(),
	  dL_dneighbor_effects.contiguous().data<float>(),
	  dL_dneighbor_shade.contiguous().data<float>(),

	  debug);
  }

  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations,dL_dneighbor_effects,dL_dneighbor_shade,dL_dgaussian_envlight);
}
